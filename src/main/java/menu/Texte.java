...
import utilitaire.ResourceUtility;
...
/**
 * Un Texte peut etre selectionnable, avoir un comportement au survol et a la
 * confirmation.
 */
public class Texte extends ElementDeMenu {
	 ...
	 */
	public Texte(final List<String> contenu, final int xDebut, final int yDebut, final boolean selectionnable,
			final List<Commande> c1, final List<Commande> c2, final int id) {
	 ...
	 * @param id              identifiant de l'ElementDeMenu
	 */
	public Texte(final List<String> contenu, final int xDebut, final int yDebut, final Taille taille,
			final int largeurMaximale, final boolean selectionnable, final List<Commande> c1, final List<Commande> c2,
			final int id) {
	 ...
	 */
	public Texte(final List<String> contenu, final int xDebut, final int yDebut, final Taille taille,
			final int largeurMaximale, final boolean selectionnable, final int opacite, final List<Commande> c1,
			final List<Commande> c2, final int id) {
	 ...
	 * @param contenu du Texte (dans plusieurs langues)
	 */
	public Texte(final List<String> contenu) {
	 ...
	 * 
	 * @param contenu       du Texte (dans plusieurs langues)
	 * @param couleurForcee pour avoir un texte d'une autre couleur que celle par
	 *                      defaut
	 */
	public Texte(final List<String> contenu, final Color couleurForcee) {
	 ...
	 */
	public Texte(final List<String> contenu, final Color couleurForcee, final Color couleurContour,
			final Taille taille) {
	...
	public final BufferedImage texteToImage() {
		...
		if (this.largeurMaximale > 0) {
			...
			for (int i = 0; i < nombreDeLignes; i++) {
				...
				if (ligne.length() > this.largeurMaximale) {
					...
					nombreDeLignes++;
					LOG.debug("La ligne \"" + ligne + "\" a ete scindee : " + textesListe.get(i) + "|"
							+ textesListe.get(i + 1));
				}
			}
		...
		g2d.dispose();
		final int hauteurTotaleImage = (hauteurDesLignes + Texte.INTERLIGNE) * nombreLignes - Texte.INTERLIGNE; // on
																												// retire
																												// l'interligne
																												// inutile
																												// tout
		...
		for (int i = 0; i < nombreLignes; i++) {
			...
			for (int j = 0; j < nombreDeBoutsDeLigne; j++) {
				...
				if (boutsDeLigne[j].matches("^\\[[0-9]+\\].*")) {
					final int numeroCouleur = Integer.parseInt(boutsDeLigne[j].split("\\[", -1)[1].split("\\]", -1)[0]);
	 ...
	 * 
	 * @param o soit le texte dans une langue unique au format String, soit le texte
	 *          multilingue ArrayList<String>
	...
	private static JSONObject chargerInformationsPolice() {
		try {
			return InterpreteurDeJson.ouvrirJson("police", "Data/");
		} catch (Exception e) {
	...
	private static String chargerNomPolice() {
		...
		if (verifierSiLaPoliceEstInstallee(police)) {
			...
			try {
				...
				ge.registerFont(ResourceUtility.loadFont("Graphics/Fonts/" + police + ".ttf"));
