...
public abstract class Graphismes {
	...
	public static BufferedImage ouvrirImage(final String dossier, final String nom) throws IOException {
		...
		var classLoader = Graphismes.class.getClassLoader();

		try (var inputStream = classLoader.getResourceAsStream("Graphics/" + dossierSlashNom)) {

			return convertirEnImageCompatible(ImageIO.read(requireNonNull(inputStream)), dossierSlashNom);
